...
void check_boundaries(t_game *game, int y, int x)
{
	if (y < 0 || x < 0 || game->map->map_line[y] == NULL || game->map->map_line[y][x] == '\0')
		handle_map_error(game, NULL, "Map is not closed/surrounded by walls");
	...
	if (c == '0' || c == 'N' || c == 'S' || c == 'E' || c == 'W')
	{
		...
		handle_map_error(game, NULL, "Map contains open space accessible from player area");
}
